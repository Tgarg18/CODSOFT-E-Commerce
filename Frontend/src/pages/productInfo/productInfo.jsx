...
const productInfo = () => {
  ...
  useEffect(()=>{
    (async()=>{
      try{
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/products/getProductInfo/${productId}`)
  ...
  const token = localStorage.getItem('token')
   async  function handleAddToCart(){
    try{
      ...
      const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/users/addToCart`,{
        data:{
